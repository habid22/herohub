...
app.get('/api/lists/:name/visibility', (req, res) => {
  ...
  const visibilityValue = lists[name].isVisible || false;
...
app.put('/api/lists/:listName/comments/:commentIndex/visibility/true', (req, res) => {
  ...
  comments[commentIndex].visibility = true;

  // Write the updated lists to the file without updating date_modified
  writeListsToFile(lists);

  res.send(`Visibility of comment at index ${commentIndex} in ${listName} set to true`);
});
